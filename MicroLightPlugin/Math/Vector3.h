...
inline float ModuleFloat(const float * v)
{
	return sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
}

inline float ModuleVector3(const Vector3& rhs)
{
...
inline float Angle(const float * a, const float * b)
{
	...
	return acos(Dot(a, b) / (ModuleFloat(a) * ModuleFloat(b)));
}
inline float Angle(const Vector3& lhs, const Vector3& rhs)
{
	...
	return acos(Dot(lhs, rhs) / (ModuleVector3(lhs) * ModuleVector3(rhs)));
}

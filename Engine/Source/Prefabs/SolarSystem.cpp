...
{
	void SolarSystem::Create(float radius, float slices, float stacks)
	{
	...
	void SolarSystem::Draw(glm::mat4 view_matrix, glm::mat4 projection_matrix) {
		GLuint sun_shader_program = Shader::GetShader("sun_shader");
		GLuint earth_shader_program = Shader::GetShader("earth_shader");
		...
		*/
		{
			...
			mvp = projection_matrix * view_matrix * modelMatrix;
		...
		DrawPlanet(earth_shader_program, "mercury", view_matrix, projection_matrix, 88, 40, 4.5f, mercury_rotation_);
		DrawPlanet(earth_shader_program, "venus", view_matrix, projection_matrix, 225, 75, 6.2f, venus_rotation_);
		...
		*/
		{
			...
			ConversionMatrix = glm::inverse(view_matrix);
			CameraPos = (glm::vec3) ConversionMatrix[3];

			glm::vec4 eyePosW = glm::vec4(CameraPos, 1);
			mvp = view_matrix*projection_matrix* modelMatrix;
		...
		*/
		...
		DrawPlanet(earth_shader_program, "mars", view_matrix,projection_matrix, 686, 180, 9.5f, mars_rotation_);
		DrawPlanet(earth_shader_program, "jupiter", view_matrix, projection_matrix, 4380, 250, 100.0f, jupiter_rotation_);
		DrawPlanet(earth_shader_program, "saturn", view_matrix, projection_matrix, 10585, 350, 90.0f, saturn_rotation_);
		DrawPlanet(earth_shader_program, "uranus", view_matrix, projection_matrix, 30660, 445, 40.0f, uranus_rotation_);
		DrawPlanet(earth_shader_program, "neptune", view_matrix, projection_matrix, 60225, 550, 40.0f, neptune_rotation_);
	...
	void SolarSystem::DrawPlanet(GLuint shader_program, std::string planet_texture,
